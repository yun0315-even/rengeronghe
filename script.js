document.addEventListener('DOMContentLoaded', () => {
        ...
        if ('webkitSpeechRecognition' in window) {
            ...
            recognition.onresult = function(event) {
                ...
                if (result.includes('打开礼物盒') || result.includes('open the gift')) {
                    // 使用千问API生成的关键词（与transformGiftBoxRandom互斥）
                    transformGiftBox();
                    // 解开下方注释 使用随机语句，随机语句在transformGiftBoxRandom中
                    // transformGiftBoxRandom();
                }
            };
    ...
    function transformGiftBox() {
        console.log('transformGiftBox-script');
    ...
    function transformGiftBoxRandom() {
        console.log('transformGiftBoxRandom-script');
        const giftBox = document.querySelector('.gift-box');
        const giftImg = document.getElementById('giftBoxImg');
        const giftTip = document.querySelector('.gift-tip');
    ...
    function setShowKeywords(keywords) {
        // 添加书写动画效果
        setTimeout(() => {
            const msgDiv = document.getElementById('envelopeMessage');
            console.log('envelopeMessage div:', msgDiv); // 调试输出
            if (msgDiv) {
                msgDiv.innerHTML = `<span class="writing-animation">${keywords}</span>`;
                msgDiv.classList.add('show');
                msgDiv.style.display = 'block';
                msgDiv.style.zIndex = 99999;
            } else {
                alert('envelopeMessage 没找到！');
            }
        }, 100);
    }
